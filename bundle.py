...
required=[p/'css/styles.css',p/'js/lessons.js',p/'js/simulations.js',p/'js/quiz-flow.js']
for f in required:
    if not f.exists():
        raise FileNotFoundError(f'Missing {f}. Run git pull or download OPEN-AIRFLOW-ACADEMY.html from README.')
...
for n in ['index.html','standalone.html','OPEN-AIRFLOW-ACADEMY.html']:
    (p/n).write_text(html)
print('OK', len(html), 'bytes ->', ', '.join(['index.html','OPEN-AIRFLOW-ACADEMY.html']))
